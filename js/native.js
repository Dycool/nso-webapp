...
(() => {
    ...
    function ensureScreens() {
        ...
        screenShell('opAlbumSettingsPage', 'Album Settings', '<div id="opAlbumSettingsBody"></div>');
    ...
    function wireScreenBackNavigation() {
        const parents = {
            ...
            opAlbumSettingsPage: 'opSettingsPage',
    ...
    function applyDarkMode(mode = localSetting('dark_mode', 'system')) {
        ...
        try {
            if (typeof initDockIconPlayers === 'function') initDockIconPlayers();
        } catch { }
    }
    ...
    function openAlbumSettings() {
        ensureScreens();
        const body = $('opAlbumSettingsBody');
        if (!body) return;

        function renderAlbumSettingsContent() {
            const isCaptured = typeof window.albumPreserveCaptureDate === 'function'
                ? window.albumPreserveCaptureDate()
                : (typeof albumPreserveCaptureDate === 'function' ? albumPreserveCaptureDate() : false);
            body.innerHTML = `
                <div class="op-radio-list">
                    <button type="button" class="op-check-row" id="opAlbumDateDownloaded">
                        <span>${escapeHtml(trKey('Album_Save_Date_Downloaded'))}</span>
                        <i class="fa-solid fa-check op-radio-check ${!isCaptured ? '' : 'hidden'}"></i>
                    </button>
                    <button type="button" class="op-check-row" id="opAlbumDateCaptured">
                        <span>${escapeHtml(trKey('Album_Save_Date_Captured'))}</span>
                        <i class="fa-solid fa-check op-radio-check ${isCaptured ? '' : 'hidden'}"></i>
                    </button>
                </div>
                <p class="op-muted">${escapeHtml(trKey('Album_Save_Date_Notice_Android'))}</p>`;

            $('opAlbumDateDownloaded')?.addEventListener('click', () => {
                const setter = window.setAlbumPreserveCaptureDate || setAlbumPreserveCaptureDate;
                if (typeof setter === 'function') setter(false);
                renderAlbumSettingsContent();
                toast(tr('Setting changed.'));
            });
            $('opAlbumDateCaptured')?.addEventListener('click', () => {
                const setter = window.setAlbumPreserveCaptureDate || setAlbumPreserveCaptureDate;
        ...
        renderAlbumSettingsContent();
        openScreen('opAlbumSettingsPage');
        applyAppLanguage($('opAlbumSettingsPage'));
    }
    ...
    function renderSettingsPage() {
        const version = (() => { try { return typeof ZNCA_VERSION !== 'undefined' ? ZNCA_VERSION : '3.5.0'; } catch { return '3.5.0'; } })();
        ...
        $('opSettingsBody').innerHTML = `
            ...
            <section class="op-group">
                <h4>Album</h4>
                <button class="op-row" id="opSettingsAlbum"><span><b>${escapeHtml(trKey('AppSettings_Album_Save_Date'))}</b></span><i class="fa-solid fa-chevron-right"></i></button>
            </section>
            <section class="op-group">
                <h4>System</h4>
        ...
        $('opSettingsAlbum')?.addEventListener('click', openAlbumSettings);
